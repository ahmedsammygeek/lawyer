			...
			<div class="col-md-12">
				<form class="card" method='POST' action='{{ route('board.goals.update', $goal ) }}' enctype="multipart/form-data" >
					...
					<div class="card-header bg-primary">
						<h3 class="card-title text-white"> تعديل بيانات  </h3>
					</div>
					<div class="card-body">
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('title_ar') is-invalid @enderror " name='title_ar' value="{{ $goal->getTranslation('title' , 'ar') }}" >
								@error('title_ar')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<textarea  class="form-control @error('content_ar') is-invalid @enderror " name='content_ar'  cols="30" rows="10"> {{ $goal->getTranslation('content' , 'ar') }} </textarea>
								@error('content_ar')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('title_en') is-invalid @enderror " name='title_en' value="{{ $goal->getTranslation('title' , 'en') }}" >
								@error('title_en')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
